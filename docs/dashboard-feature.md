...
1. The menu already links to `Dashboard`, but the page does not exist.
   - navigation is already wired
   - implementation must create the page before the branch is usable

2. Aggregate correctness depends on fetch size.
...
6. Use `photo_card` as the canonical front-photo field for the "missing front" metric
